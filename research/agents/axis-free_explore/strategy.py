...
class Strategy:
    ...
    def __init__(self, config: dict | None = None):
        ...
        self.spread_frac = float(cfg.get("spread_frac", 0.6932442527025255))
        # Inventory skew strength (price shift per share of net YES exposure) — stronger
        self.skew_bps_per_share = float(cfg.get("skew_bps_per_share", 0.25378545021418314))
        # Target quote size as multiple of min_size — close to min
        self.size_mult = float(cfg.get("size_mult", 1.2570775316164875))
        # Hard inventory cap (shares of YES-equivalent net) — much tighter than v1 (~800)
        self.max_abs_inv = float(cfg.get("max_abs_inv", 150.0))
        # Soft inventory cap — much tighter than v1 (~349)
        self.inv_soft_cap = float(cfg.get("inv_soft_cap", 20.0))
        ...
        self.inv_pause_ticks = int(cfg.get("inv_pause_ticks", 16))
        # Optional allowlist: only quote if daily_reward_pool >= threshold
        self.min_daily_reward_pool = float(cfg.get("min_daily_reward_pool", 300.0))
        self.enforce_pool_allowlist = bool(cfg.get("enforce_pool_allowlist", True))
        # Low-pool size haircut when allowlist not enforced
        self.low_pool_size_frac = float(cfg.get("low_pool_size_frac", 0.5))
        ...
        self.pull_size_mult = float(cfg.get("pull_size_mult", 0.4287129142067908))
