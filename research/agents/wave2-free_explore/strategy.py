...
class Strategy:
    ...
    def __init__(self, config: dict | None = None):
        ...
        self.spread_frac = float(cfg.get("spread_frac", 0.6706054520400928))
        # Inventory skew strength (price shift per share of net YES exposure) — stronger
        self.skew_bps_per_share = float(cfg.get("skew_bps_per_share", 0.15759260566408342))
        # Target quote size as multiple of min_size — close to min
        self.size_mult = float(cfg.get("size_mult", 1.222984995761312))
        ...
        self.inv_soft_cap = float(cfg.get("inv_soft_cap", 30.0))
        ...
        self.inv_pause_ticks = int(cfg.get("inv_pause_ticks", 6))
        # Optional allowlist: only quote if daily_reward_pool >= threshold
        self.min_daily_reward_pool = float(cfg.get("min_daily_reward_pool", 175.0))
        ...
        self.mid_move_hard_cancel_cents = float(cfg.get("mid_move_hard_cancel_cents", 6.0))
