...
class Strategy:
    ...
    def __init__(self, config: dict | None = None):
        ...
        self.spread_frac = float(cfg.get("spread_frac", 0.7388290545308196))
        # Inventory skew strength (price shift per share of net YES exposure) — stronger
        self.skew_bps_per_share = float(cfg.get("skew_bps_per_share", 0.27330333997205347))
        # Target quote size as multiple of min_size — close to min
        self.size_mult = float(cfg.get("size_mult", 1.1616762829404854))
        # Hard inventory cap (shares of YES-equivalent net) — much tighter than v1 (~800)
        self.max_abs_inv = float(cfg.get("max_abs_inv", 200.0))
        # Soft inventory cap — much tighter than v1 (~349)
        self.inv_soft_cap = float(cfg.get("inv_soft_cap", 50.0))
        ...
        self.mid_move_cancel_cents = float(cfg.get("mid_move_cancel_cents", 1.0))
        ...
        self.inv_pause_ticks = int(cfg.get("inv_pause_ticks", 8))
        # Optional allowlist: only quote if daily_reward_pool >= threshold
        self.min_daily_reward_pool = float(cfg.get("min_daily_reward_pool", 200.0))
