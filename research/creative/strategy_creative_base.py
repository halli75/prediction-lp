"""
Creative structural modes forked from wave5b_pull24_p46_sf672.

Champion knobs preserved as defaults. Creative modes are OFF by default so
overlay-json can enable them one-at-a-time or in combos.

Modes:
  1) force_flatten     — early pay-to-exit reduce-only when |net| large
  2) day_boundary_flatten — UTC day-change → flatten until |net| small
  3) toxicity_blackout — after inv growth, adverse mid → both-side stand-down
  4) dynamic_top_k     — each UTC day only quote top-K by daily_reward_pool
  5) harvest_or_flatten — flat harvest two-sided; else never add
  WAVE3:
  6) session_flatten   — flatten at UTC session boundaries (00/12 or every N hours)
  7) inventory_age_flatten — reduce-only after |net| held away from 0 too long
  8) overnight_quiet   — UTC overnight: reduce-only or size_mult shrink
...
class Strategy:
    ...
    def __init__(self, config: dict | None = None):
        ...
        self.day_flatten = bool(cfg.get("day_flatten", False))
        ...
        self.overnight_quiet = bool(cfg.get("overnight_quiet", False))
        self.overnight_start_hour = int(cfg.get("overnight_start_hour", 0))
        self.overnight_end_hour = int(cfg.get("overnight_end_hour", 6))  # [start, end)
