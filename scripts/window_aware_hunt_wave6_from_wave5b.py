#!/usr/bin/env python3
"""Wave6: search from wave5b champion under updated Aug10 liveish gates."""
...
AGENT_DIR = ROOT / "research/agents/wave6-from-wave5b"
STRAT = AGENT_DIR / "strategy.py"
OUT = ROOT / "results/window_aware_hunt_wave6_from_wave5b.json"
...
JOINT_MUTATIONS = [
    ("sf671_pull24_p46", [("spread_frac", 0.671), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464)]),
    ("sf673_pull24_p46", [("spread_frac", 0.673), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464)]),
    ("sf674_pull24_p46", [("spread_frac", 0.674), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464)]),
    ("sf672_pull22_p46", [("spread_frac", 0.672), ("inv_pause_ticks", 22), ("pull_size_mult", 0.464)]),
    ("sf672_pull26_p46", [("spread_frac", 0.672), ("inv_pause_ticks", 26), ("pull_size_mult", 0.464)]),
    ("sf672_pull24_p45", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.45)]),
    ("sf672_pull24_p48", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.48)]),
    ("sf672_pull24_p46_size127", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("size_mult", 1.27)]),
    ("sf672_pull24_p46_size130", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("size_mult", 1.30)]),
    ("sf670_pull24_p47", [("spread_frac", 0.670), ("inv_pause_ticks", 24), ("pull_size_mult", 0.47)]),
    ("sf675_pull20_p46", [("spread_frac", 0.675), ("inv_pause_ticks", 20), ("pull_size_mult", 0.464)]),
    ("sf672_pull24_p46_skew32", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("skew_bps_per_share", 0.32)]),
    ("sf672_pull24_p46_skew38", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("skew_bps_per_share", 0.38)]),
    ("sf672_pull28_p44", [("spread_frac", 0.672), ("inv_pause_ticks", 28), ("pull_size_mult", 0.44)]),
    ("sf673_pull24_p46_size128", [("spread_frac", 0.673), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("size_mult", 1.28)]),
    ("near045_pull24_sf672", [("near_mid_size_mult", 0.45), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("spread_frac", 0.672)]),
    ("sf672_pull24_p46_widen17", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("mid_move_widen_mult", 1.7)]),
    ("sf6715_pull25_p455", [("spread_frac", 0.6715), ("inv_pause_ticks", 25), ("pull_size_mult", 0.455)]),
]
...
def do_promote(trial, label):
    ...
    promo = {
        "job": f"wave6_promotion_{label}", "label": label, "parent": "wave5b_pull24_p46_sf672", "eval": "liveish",
    ...
    if isinstance(findings, list):
        findings.append({"ts": data["updated_at"], "agent": "wave6-from-wave5b", "event": "promoted_liveish_aug10", "label": label})
        data["findings"] = findings[-200:]
    path.write_text(json.dumps(data, indent=2, default=str))
    with open(ROOT / "results/overnight_digest.md", "a") as f:
        f.write(f"\n\n## Wave6 PROMOTION — {label} — {datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')}\n\n- Knobs: `{trial.get('knobs')}`\n- Liveish sample **{trial['sample_net_pnl']:.2f}** / 30d **{trial['net_30d']:.2f}** / 60d **{trial['net_60d']:.2f}**\n- Beat wave5b Aug10 gates. Champion synced.\n")


def main():
    AGENT_DIR.mkdir(parents=True, exist_ok=True)
    shutil.copy2(PARENT, STRAT)
    state = {"trials": [], "wave": "wave6", "phase": "from_wave5b_liveish_aug10"}
    trials = state["trials"]
    best = None
    promoted = False
    t0 = time.time()
    print(f"Wave6 from wave5b gates S>{CHAMP_SAMPLE} 30>{CHAMP_30} 60>{CHAMP_60}", flush=True)
    for i, (mlabel, knobs) in enumerate(JOINT_MUTATIONS, 1):
        ...
        if "max_abs_inv" in knobs_d and float(knobs_d["max_abs_inv"]) > 100:
            continue
        print(f"\n=== TRY 6-{i}/{len(JOINT_MUTATIONS)}: {mlabel} {knobs} ===", flush=True)
        trial = {"try": 7000+i, "phase": "wave6_from_wave5b", "mutation_label": mlabel, "knobs": knobs,
