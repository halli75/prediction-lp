#!/usr/bin/env python3
"""Wave7: joint hunt from wave5b champion under Aug10 liveish gates (post wave6 reject)."""
...
AGENT_DIR = ROOT / "research/agents/wave7-from-wave5b"
STRAT = AGENT_DIR / "strategy.py"
OUT = ROOT / "results/window_aware_hunt_wave7_from_wave5b.json"
...
JOINT_MUTATIONS = [
    ("sf665_mhs018_pull24_p46", [("spread_frac", 0.665), ("min_half_spread", 0.018), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464)]),
    ("sf668_mhs018_pull24_p46", [("spread_frac", 0.668), ("min_half_spread", 0.018), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464)]),
    ("sf670_mhs02_pull24_p46", [("spread_frac", 0.670), ("min_half_spread", 0.02), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464)]),
    ("sf672_mhs018_pull24_p47", [("spread_frac", 0.672), ("min_half_spread", 0.018), ("inv_pause_ticks", 24), ("pull_size_mult", 0.47)]),
    ("sf672_pull24_p475", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.475)]),
    ("sf672_pull23_p47", [("spread_frac", 0.672), ("inv_pause_ticks", 23), ("pull_size_mult", 0.47)]),
    ("sf671_pull24_p47", [("spread_frac", 0.671), ("inv_pause_ticks", 24), ("pull_size_mult", 0.47)]),
    ("sf673_pull24_p47", [("spread_frac", 0.673), ("inv_pause_ticks", 24), ("pull_size_mult", 0.47)]),
    ("sf672_pull24_p46_skew34", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("skew_bps_per_share", 0.34)]),
    ("sf672_pull24_p46_skew36", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("skew_bps_per_share", 0.36)]),
    ("sf666_pull25_p46", [("spread_frac", 0.666), ("inv_pause_ticks", 25), ("pull_size_mult", 0.464)]),
    ("sf672_pull24_p46_size129", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("size_mult", 1.29)]),
    ("near048_sf672_pull24", [("near_mid_size_mult", 0.48), ("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464)]),
    ("near042_sf671_pull24", [("near_mid_size_mult", 0.42), ("spread_frac", 0.671), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464)]),
    ("sf672_pull27_p46", [("spread_frac", 0.672), ("inv_pause_ticks", 27), ("pull_size_mult", 0.464)]),
    ("sf669_pull24_p465", [("spread_frac", 0.669), ("inv_pause_ticks", 24), ("pull_size_mult", 0.465)]),
    ("sf672_pull24_p46_widen165", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("mid_move_widen_mult", 1.65)]),
    ("sf671_pull24_p46_size1285", [("spread_frac", 0.671), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("size_mult", 1.285)]),
    ("sf674_pull24_p455", [("spread_frac", 0.674), ("inv_pause_ticks", 24), ("pull_size_mult", 0.455)]),
    ("sf672_mhs016_pull24_p48", [("spread_frac", 0.672), ("min_half_spread", 0.016), ("inv_pause_ticks", 24), ("pull_size_mult", 0.48)]),
]
...
def do_promote(trial, label):
    ...
    promo = {
        "job": f"wave7_promotion_{label}", "label": label, "parent": "wave5b_pull24_p46_sf672", "eval": "liveish",
...
**Current champion:** `{label}` (liveish-validated Aug10; wave7 `{trial.get('mutation_label')}`)
...
"""
    ...
    data["current_champion"] = {"label": label, "eval": "liveish_aug10", "wave": "wave7", "mutation_label": trial.get("mutation_label"),
    ...
    if isinstance(findings, list):
        findings.append({"ts": data["updated_at"], "agent": "wave7-from-wave5b", "event": "promoted_liveish_aug10", "label": label})
        data["findings"] = findings[-200:]
    path.write_text(json.dumps(data, indent=2, default=str))
    with open(ROOT / "results/overnight_digest.md", "a") as f:
        f.write(f"\n\n## Wave7 PROMOTION — {label} — {datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')}\n\n- Knobs: `{trial.get('knobs')}`\n- Liveish sample **{trial['sample_net_pnl']:.2f}** / 30d **{trial['net_30d']:.2f}** / 60d **{trial['net_60d']:.2f}**\n- Beat wave5b Aug10 gates. Champion synced.\n")


def main():
    AGENT_DIR.mkdir(parents=True, exist_ok=True)
    shutil.copy2(PARENT, STRAT)
    state = {"trials": [], "wave": "wave7", "phase": "from_wave5b_liveish_aug10"}
    trials = state["trials"]
    best = None
    promoted = False
    t0 = time.time()
    print(f"Wave7 from wave5b gates S>{CHAMP_SAMPLE} 30>{CHAMP_30} 60>{CHAMP_60}", flush=True)
    for i, (mlabel, knobs) in enumerate(JOINT_MUTATIONS, 1):
        ...
        trial = {"try": 8000+i, "phase": "wave7_from_wave5b", "mutation_label": mlabel, "knobs": knobs,
    ...
    print(f"\nWave7 done promoted={promoted} n={len(trials)} best={best.get('mutation_label') if best else None}", flush=True)
