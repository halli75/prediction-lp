#!/usr/bin/env python3
"""Wave8: fine ridge hunt from wave5b under Aug10 liveish gates (post wave6/7 near-misses)."""
...
AGENT_DIR = ROOT / "research/agents/wave8-from-wave5b"
STRAT = AGENT_DIR / "strategy.py"
OUT = ROOT / "results/window_aware_hunt_wave8_from_wave5b.json"
...
JOINT_MUTATIONS = [
    # Finer ridge around sf673 / pull0.46–0.47 (wave6/7 near-misses: +30d tiny / 60d ~flat)
    ("sf6722_pull24_p462", [("spread_frac", 0.6722), ("inv_pause_ticks", 24), ("pull_size_mult", 0.462)]),
    ("sf6725_pull24_p462", [("spread_frac", 0.6725), ("inv_pause_ticks", 24), ("pull_size_mult", 0.462)]),
    ("sf6728_pull24_p46", [("spread_frac", 0.6728), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464)]),
    ("sf673_pull24_p462", [("spread_frac", 0.673), ("inv_pause_ticks", 24), ("pull_size_mult", 0.462)]),
    ("sf673_pull24_p458", [("spread_frac", 0.673), ("inv_pause_ticks", 24), ("pull_size_mult", 0.458)]),
    ("sf6732_pull24_p46", [("spread_frac", 0.6732), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464)]),
    ("sf6735_pull24_p46", [("spread_frac", 0.6735), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464)]),
    ("sf673_pull23_p46", [("spread_frac", 0.673), ("inv_pause_ticks", 23), ("pull_size_mult", 0.464)]),
    ("sf673_pull25_p46", [("spread_frac", 0.673), ("inv_pause_ticks", 25), ("pull_size_mult", 0.464)]),
    ("sf6725_pull24_p468", [("spread_frac", 0.6725), ("inv_pause_ticks", 24), ("pull_size_mult", 0.468)]),
    # Compensating: mild spread up + slightly stronger pull (60d helper)
    ("sf673_pull24_p455", [("spread_frac", 0.673), ("inv_pause_ticks", 24), ("pull_size_mult", 0.455)]),
    ("sf674_pull24_p45", [("spread_frac", 0.674), ("inv_pause_ticks", 24), ("pull_size_mult", 0.45)]),
    ("sf672_pull24_p45", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.45)]),
    # Skew sample keep (~0.50) with wave5b pull/spread
    ("sf672_pull24_p46_skew50", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("skew_bps_per_share", 0.50)]),
    ("sf673_pull24_p46_skew50", [("spread_frac", 0.673), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("skew_bps_per_share", 0.50)]),
    ("sf672_pull24_p46_skew48", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("skew_bps_per_share", 0.48)]),
    # Size haircut with pause/pull lock
    ("sf672_pull24_p46_size1288", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("size_mult", 1.288)]),
    ("sf673_pull24_p46_size1288", [("spread_frac", 0.673), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("size_mult", 1.288)]),
    # Soft cancel widen mild
    ("sf672_pull24_p46_widen162", [("spread_frac", 0.672), ("inv_pause_ticks", 24), ("pull_size_mult", 0.464), ("mid_move_widen_mult", 1.62)]),
    ("sf673_pull24_p462_pause23", [("spread_frac", 0.673), ("inv_pause_ticks", 23), ("pull_size_mult", 0.462)]),
]
...
def do_promote(trial, label):
    ...
    promo = {
        "job": f"wave8_promotion_{label}", "label": label, "parent": "wave5b_pull24_p46_sf672", "eval": "liveish",
...
**Current champion:** `{label}` (liveish-validated Aug10; wave8 `{trial.get('mutation_label')}`)
...
"""
    ...
    data["current_champion"] = {"label": label, "eval": "liveish_aug10", "wave": "wave8", "mutation_label": trial.get("mutation_label"),
    ...
    if isinstance(findings, list):
        findings.append({"ts": data["updated_at"], "agent": "wave8-from-wave5b", "event": "promoted_liveish_aug10", "label": label})
        data["findings"] = findings[-200:]
    path.write_text(json.dumps(data, indent=2, default=str))
    with open(ROOT / "results/overnight_digest.md", "a") as f:
        f.write(f"\n\n## Wave8 PROMOTION — {label} — {datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')}\n\n- Knobs: `{trial.get('knobs')}`\n- Liveish sample **{trial['sample_net_pnl']:.2f}** / 30d **{trial['net_30d']:.2f}** / 60d **{trial['net_60d']:.2f}**\n- Beat wave5b Aug10 gates. Champion synced.\n")


def main():
    AGENT_DIR.mkdir(parents=True, exist_ok=True)
    shutil.copy2(PARENT, STRAT)
    state = {"trials": [], "wave": "wave8", "phase": "from_wave5b_liveish_aug10"}
    trials = state["trials"]
    best = None
    promoted = False
    t0 = time.time()
    print(f"Wave8 from wave5b gates S>{CHAMP_SAMPLE} 30>{CHAMP_30} 60>{CHAMP_60}", flush=True)
    for i, (mlabel, knobs) in enumerate(JOINT_MUTATIONS, 1):
        ...
        trial = {"try": 9000+i, "phase": "wave8_from_wave5b", "mutation_label": mlabel, "knobs": knobs,
    ...
    print(f"\nWave8 done promoted={promoted} n={len(trials)} best={best.get('mutation_label') if best else None}", flush=True)
